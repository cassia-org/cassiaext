...
# Check arguments for if we should clean
clean = False
for arg in sys.argv:
    if arg == "--clean":
        clean = True
...
for dep in deps:
    ...
    if clean:
        ...
        if (subprocess.run(["git", "clean", "-fdx"], stdout=sys.stdout, cwd=os.path.join(script_dir, dep.name)).returncode != 0):
            print(f"Error: Failed to clean {dep.name}")
            exit(1)
