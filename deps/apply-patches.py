...
class Patch:
    def __init__(self, path, name, gitignore=False):
        self.path = path
        self.name = name
...
for file in os.listdir(os.path.join(script_dir, "patches", "gitignore")):
    if file.endswith(".patch"):
        patches.append(Patch(os.path.join(script_dir, "patches", "gitignore", file), file, True))
...
# Set gitignore on dependencies that only have gitignore patches
for dep in deps:
    if len(dep.patches) > 0 and dep.patches[0].gitignore:
        dep.gitignore = True
    else:
        dep.gitignore = False
...
print("Patches:")
for dep in deps:
    if not dep.gitignore:
        print(f"- {dep.name}")
        for patch in dep.patches:
            if not patch.gitignore:
